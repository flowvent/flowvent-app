# ============================================================
#  FLOWVENT — Fase 4: Categorías + base de Ranking
#  Versión: 4.0.0
# ============================================================
...
<style>
  ...
  .sb-team { font-family:'Barlow Condensed',sans-serif; font-size:1rem; font-weight:700; color:#FFF; text-transform:uppercase; line-height:1.1; }
  ...
  .rk-row { background:#0E0E1A; border-radius:10px; }
...
def load_data() -> pd.DataFrame:
    ...
    sh  = client.open_by_url(secrets["spreadsheet"])
    ws  = sh.worksheet("events")
    data = ws.get_all_records()
    if not data:
        return pd.DataFrame()
    df = pd.DataFrame(data)
    cols = ["event_id","equipo","categoria","heat","wod_nombre","arena","estado","hora_inicio","resultado","puntos","tipo_puntaje","orden_display"]
    for c in cols:
...
def cat_chip(cat: str) -> str:
    ...
    cls = {"RX":"cat-RX","SCALED":"cat-SCALED","MASTERS":"cat-MASTERS"}.get(cat,"cat-DEFAULT")
    label = cat if cat else "—"
    return f'<span class="cat-chip {cls}">{label}</span>'
...
def render_card(row: pd.Series) -> str:
    ...
    pts_val   = row.get("puntos",0)
    pts_f     = f'<div class="fv-field"><span class="fv-label">Puntos</span><span class="fv-value res-live">{int(pts_val)}</span></div>' if pts_val and float(pts_val)>0 else ""
...
def render_schedule(df):
    ...
    for hora_raw in horas:
        hora_display = parse_hora(hora_raw)
        html += f'<tr><td class="hora-td">{hora_display}</td>'
        for arena in arenas:
            mask = (df["hora_inicio"]==hora_raw)&(df["arena"]==arena)
            rows = df[mask]
            if rows.empty:
                ...
                for _, r in rows.iterrows():
                    ...
                    cat_h = f'<div style="margin-top:4px">{cat_chip(r.get("categoria",""))}</div>'
                    cells += (
...
def main():
    ...
    if df.empty:
        st.warning("La hoja 'events' está vacía. Agregá datos en Google Sheets.")
        return
    ...
    # ── Filtros comunes ──
    categorias  = sorted([c for c in df["categoria"].dropna().unique() if c and c != "NAN"])
